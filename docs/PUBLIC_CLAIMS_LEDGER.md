...
| H4 | Current official Cloudflare Web Analytics, Cloudflare Workers Logs, and Resend documentation cited in [`docs/PRIVACY_DATA_FLOW.md`](./PRIVACY_DATA_FLOW.md), checked 2026-08-12 | Public |
...
| WEB-03 | Limited contact attribution. | The site records the first path visited in a browser-tab session and, when present, utm_source, utm_medium, and utm_campaign. These values stay in that tab's session storage and are sent only with a submitted inquiry. Other query parameters are not retained for attribution. | H3 | Public | Three parameters, exact | Yes |
...
| WEB-05 | Contact-form email processors and retention. | For a valid submission, the Worker sends the form fields and limited attribution to Resend as a plain-text email. Resend's dashboard exposes the sent message and API request log, and its current documentation says it retains email data for 30 days. The destination is the Hekswerk mailbox hosted by Microsoft 365. There is no additional Hekswerk-owned contact database. | H3, H4, H5 | Public | 30 days, current provider statement | Yes. Recheck provider documentation and DNS before reuse. |
...
| WEB-07 | Website hosting. | Cloudflare Workers Static Assets serves the canonical site at www.hekswerk.com. Cloudflare processes the network request, requested path, and ordinary connection metadata needed to return the page. The same build may also be available at a workers.dev review address marked not to be indexed. | H3, H4 | Public | None | Yes. Observed after the production DNS cutover on 2026-08-12. |
...
| WEB-09 | Cloudflare Web Analytics behavior. | Cloudflare Web Analytics receives the site host and path, referring site, country, device and browser categories, and performance timing. Its current documentation says Web Analytics does not log query strings, use cookies or browser storage, or collect or use visitors' personal data. Cloudflare keeps unsampled beacon data for seven days and then retains aggregated data; its dashboard exposes the previous six months. | H3, H4 | Public | Seven days and six months, current provider statements | Yes. Attribute the claims to Cloudflare and recheck before reuse. |
| WEB-10 | Inquiry deletion request. | A person can request deletion by emailing levi@hekswerk.com, preferably from the submitted address, with the approximate submission date. I can remove copies I control from my mailbox and ask Resend about a provider-held copy that is still within its retention period. I will identify provider-controlled logs, backups, or retention that I cannot directly erase or verify. | H3, H4 | Public | None | Yes. Do not imply guaranteed deletion from provider systems. |
...
| WEB-12 | Features not present on the site. | The site does not include advertising trackers, a scheduling platform, or a generic chatbot. | H3 | Public | None | Yes. Recheck the implementation before reuse. |
